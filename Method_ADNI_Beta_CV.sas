...
PROC MIXED DATA = DataAnalysis_&Data. METHOD = ml EMPIRICAL;
  ...
  MODEL DifOutcome =  bl_Diagnostic*Visit_year / NOINT S; * Stat Medƒ‚ƒfƒ‹ *;
...
DATA CV_Train_ADNI; SET CV_Train;
  RENAME SMPE = AveragePredictiveError;
  KEEP CateG SMPE Freq Rep;
RUN;

DATA CV_Test_ADNI; SET CV_Test;
  RENAME SMPE = AveragePredictiveError;
  KEEP CateG SMPE Freq Rep;
RUN;

DATA PredError_CV_Train_ADNI; SET Sum_CV_Train;
  RENAME M_SMPE = AveragePredictiveError;
  ATTRIB _ALL_ LABEL = " " ;
RUN;

DATA PredError_CV_Test_ADNI; SET Sum_CV_Test;
  RENAME M_SMPE = AveragePredictiveError;
  ATTRIB _ALL_ LABEL = " " ;
RUN;
