...
# Outcomeの要約統計量
# subset_data <- Data_Test %>%
#   filter(Visit_year ==0 & GMM_Cluster == 3)
# 
...
Trajectories <- TrajALL

keep_objects <- c("Data_ADNI", "Data_Result","PredALL", "Outcome", "MaxTau", "seed", "Trajectories")
all_objects <- ls()
...
AveratePredictiveError <- PredALL %>%
  group_by(GMM_Cluster) %>%
  summarise(mean = mean(AveragePredictiveError, na.rm = TRUE)) # NAを無視する場合は na.rm = TRUE を指定

print(AveratePredictiveError)
