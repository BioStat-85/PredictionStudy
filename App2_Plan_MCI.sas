...
*------------------------------------------------------------------------------*;
* Patient population from (real-world) dataset who satisfy the eligibility criteria of the planned RCT *;
*------------------------------------------------------------------------------*;
DATA Data; SET Data;
...
*--------------------------------------------------------------------------------------------------------------*;
* Patient population whose cognitive test score of the primary endpoint in the planned RCT have been collected for at least several years *;
*--------------------------------------------------------------------------------------------------------------*;
DATA _Data; SET Data;
...
DATA _Temp; SET DataAnalysis(WHERE = (Visit_year = 0 AND bl_Diagnostic = "MCI")); RUN; * # *;
...
PROC MIXED DATA = DataAnalysis METHOD = ml EMPIRICAL;
  CLASS ID bl_Diagnostic(REF='MCI');
