...
MaxTau <-  # prediction period (set to 80 in the paper)
Limited_Follow_Up <- "YES" # YES or NO
...
# Temp <- Data_ADNI %>%
#   filter(!is.na(CDRSB)) # MMSE or ADAS13 or CDRSB
# result <- Temp %>%
#   group_by(ID) %>%
#   summarise(Observation_Count = n()) %>%
#   summarise(
#     Min_Observations = min(Observation_Count),
#     Max_Observations = max(Observation_Count)
#   )
# print(result)


# Summary statistics for the evaluation point
# patient_count <- table(Data_ADNI$Visit_year)
# patient_count_df <- as.data.frame(patient_count)
# colnames(patient_count_df) <- c("Visit_year", "Patient_count")
# print(patient_count_df)



# Calculate the maximum value of Visit_year for each patient
max_visit_year_per_patient <- Data_ADNI %>%
  group_by(ID) %>%
  summarise(Max_Visit_Year = max(Visit_year, na.rm = TRUE))
summary_stats <- max_visit_year_per_patient %>%
  summarise(
    Mean = mean(Max_Visit_Year, na.rm = TRUE),
    SD = sd(Max_Visit_Year, na.rm = TRUE),
    Median = median(Max_Visit_Year, na.rm = TRUE),
    Q1 = quantile(Max_Visit_Year, 0.25, na.rm = TRUE),
    Q3 = quantile(Max_Visit_Year, 0.75, na.rm = TRUE),
    Min = min(Max_Visit_Year, na.rm = TRUE),
    Max = max(Max_Visit_Year, na.rm = TRUE)
  )
print(summary_stats)
...
#   Mean = mean(selected_variable, na.rm = TRUE), # 平均値
#   SD = sd(selected_variable, na.rm = TRUE),     # 標準偏差
#   Q1 = quantile(selected_variable, 0.25, na.rm = TRUE), # 第1四分位数
#   Median = median(selected_variable, na.rm = TRUE),     # 中央値
#   Q3 = quantile(selected_variable, 0.75, na.rm = TRUE), # 第3四分位数
#   Min = min(selected_variable, na.rm = TRUE),           # 最小値
#   Max = max(selected_variable, na.rm = TRUE)            # 最大値
# )
...
for (seed in 1:1000) {
  ...
  quantiles <- quantile(max_visit_year_per_patient$Max_Visit_Year, probs = c(0.25, 0.5, 0.75))
  
  set.seed(seed)
  sampled_indices <- createDataPartition(
    y = max_visit_year_per_patient$Max_Visit_Year,
    times = 1,
    p = 98 / nrow(max_visit_year_per_patient), # 98 for N = 100 or 298 for N = 300
    list = FALSE
  )
  
  sampled_Data <- max_visit_year_per_patient[sampled_indices, ]
  sampled_quantiles <- quantile(sampled_Data$Max_Visit_Year, probs = c(0.25, 0.5, 0.75))

  extracted_Data <- Data_ADNI[Data_ADNI$ID %in% sampled_Data$ID, ]

  
  Data <- extracted_Data
  DataAnalysis <- extracted_Data
  ...
  model <- lmer(DifOutcome ~  0 + bl_Diagnostic:Visit_year + (0 + Visit_year|ID), 
                data = Data_Train, 
                control = lmerControl(optim="bobyqa"),
                REML = FALSE
  )
  
  fixed_effects <- data.frame(estimate = fixef(model))
  
  list_bl_Diagnostic_df <- data.frame(bl_Diagnostic = sort(unique(Data_Train$bl_Diagnostic)))
  ...
  list_bl_Diagnostic_df <- data.frame(bl_Diagnostic = sort(unique(Data_Test$bl_Diagnostic)))
  fixed_effect <- cbind(list_bl_Diagnostic_df, fixed_effects)
  colnames(fixed_effect) <- c("bl_Diagnostic", "Fixed_Effect")
  random_effects <- ranef(model)$ID
  random_effect <- data.frame(ID = rownames(random_effects), Random_Effect = random_effects[, "Visit_year"])
  ...
  for(i in 1:1){
    ...
    for (CateG in seq(Min_Cate, Max_Cate, 1)) {
      ...
      # Superposition of observed cognitive test scores on trajectory
      #Est_Temp <- Est %>% select(ID, GMM_Cluster)
      
      Data_Temp <- Data_Test %>% filter(GMM_Cluster == !!CateG)
      #Data_Temp <- Data %>% filter(GMM_Cluster == 1)
  ...
  keep_objects <- c("Data_ADNI","max_visit_year_per_patient", "PredALL", "Outcome", "MaxTau", "seed", "output_file_name", "Limited_Follow_Up")
  all_objects <- ls()
...
Result_SampleSize_FollowUp <- PredALL %>%
  group_by(GMM_Cluster) %>%
  summarise(
    Mean = mean(AveragePredictiveError, na.rm = TRUE),
    SD = sd(AveragePredictiveError, na.rm = TRUE),
    Median = median(AveragePredictiveError, na.rm = TRUE),
    Q1 = quantile(AveragePredictiveError, 0.25, na.rm = TRUE),
    Q3 = quantile(AveragePredictiveError, 0.75, na.rm = TRUE),
    Min = min(AveragePredictiveError, na.rm = TRUE),
    Max = max(AveragePredictiveError, na.rm = TRUE)
  )

# write_xlsx(PredALL, path = output_file_name)
print(Result_SampleSize_FollowUp)
